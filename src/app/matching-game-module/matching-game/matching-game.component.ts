...
export class MatchingGameComponent {
  ...
  constructor( private categoryService: CategoryService, private dialog: MatDialog, private router: Router, private GamePointsService: GamePointsService){}
...
private loadCurrentCategory(): void {
  ...
  if (this.currentCategoryId) {
    this.currentCategory = this.categoryService.get(parseInt(this.currentCategoryId));
...
private initializeGameWords(category: Category): void {
  ...
  const tempWords = this.selectRandomWords(category.words, MatchingGameComponent.WORDS_PER_GAME);
  console.log('Selected words:', tempWords);
  // Assign English words directly
  this.chosenWordsEnglish = tempWords.map(word => ({ origin: word.origin, target: word.target }));
  // Shuffle only the Hebrew translations using Math.random() - 0.5
  this.chosenWordsHebrew = tempWords.map(word => word.target).sort(() => Math.random() - 0.5);
...
private updateWordStatusToSelected(index: number, wordStatus: WordStatus[]): void {
  const selectedIndex = wordStatus.findIndex(status => status == WordStatus.Selected);
  if (selectedIndex > -1) {
...
private updateWordStatusToDisabled(index: number, wordStatus: WordStatus[]): void {
...
private completedGameDialog(): void {
  this.openDialog('WELL DONE!', `You managed to match ${MatchingGameComponent.WORDS_PER_GAME} pairs of words and you've finished the game!` ,'SHOW GAME SUMMARY', 'gameover');
}

private showMatchingDialog(): void {
  this.openDialog('MATCHED!', `You have matched ${this.totalSuccess} words.`, 'CONTINUE', 'matched');
}

private showWrongDialog(): void {
  this.openDialog('WRONG!', `You failed matching the words. You have ${this.totalAttempts} attempts so far.`, 'TRY AGAIN!', 'wrong');
}

handleWordClicked(index: number): void {
  let englishWordIndex = this.englishWordStatus.findIndex(status => status == WordStatus.Selected);
  let hebrewWordIndex = this.hebrewWordStatus.findIndex(status => status == WordStatus.Selected);
  ...
  if (this.englishWordStatus[index] !== WordStatus.Disabled) {
    ...
    if (hebrewWordIndex !== -1) {
      ...
      if (chosenWordInEnglish !== chosenWordInHebrew) {
        ...
        this.updateWordStatusToDisabled(hebrewWordIndex, this.hebrewWordStatus);
        this.calculatePoints(true);
        this.totalSuccess++;
        this.showMatchingDialog();
        if (this.isGameOver()) {
          this.completedGameDialog();
          this.GamePointsService.addGamePlayed(new GamePlayed(this.currentCategory!.id, 1, this.totalPoints));
        }
      }
    }
  }
...
handleHebrewWordClicked(index: number): void {
  ...
  if (this.hebrewWordStatus[index] !== WordStatus.Disabled) {
    ...
    if (englishWordIndex !== -1) {
      let chosenWordInEnglish = this.chosenWordsEnglish[englishWordIndex].target;
      ...
      if (chosenWordInHebrew !== chosenWordInEnglish) {
        ...
        this.updateWordStatusToDisabled(englishWordIndex, this.englishWordStatus);
...
openDialog (title: string, message: string, buttonText: string, dialogType: string) {
